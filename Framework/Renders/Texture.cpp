	...
	: file(file)
{
	...
	if(dir !=L"")
		String::Replace(&this->file, dir, L"");
}
...
{	
	...
	for (UINT y = 0; y < desc.Height; y++)
	{
		for (UINT x = 0; x < desc.Width; x++)
		{
			...
			pixels->push_back(D3DXCOLOR(a, b, g, r)*f);
		}
	}
	...
	sView(NULL), sProjection(NULL)
{
	...
	//Create Texture2D -RTV
	{
		...
		desc.MiscFlags = D3D11_RESOURCE_MISC_TEXTURECUBE;	/* 큐브 만드는 애 */
		desc.MipLevels = 1;
		desc.SampleDesc.Count = 1;
	...
	//Create SRV
	{
		...
		desc.Format = rtvFormat;
		desc.ViewDimension = D3D11_SRV_DIMENSION_TEXTURECUBE;	/*큐브라서*/
...
void TextureCube::Position(Vector3 & position)
{
	...
	/* 정투영 */
	perspective = new Perspective(1, 1, 0.1f, 500.f, Math::PI*0.5f);
