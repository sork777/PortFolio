...
void Shader::CreateEffect()
{
	...
	//fxo가 아니면 fxo로 변경
	if (Path::GetExtension(file).compare(L"fxo") != 0)
		file = L"../../_Shaders/" + Path::GetFileNameWithoutExtension(file) + L".fxo";
	//fxo 없으면 다시 오리지널 패스로
	...
	else if (Path::GetExtension(file) == L"fxo")
	{
		...
		fileHandle = NULL;
		//컴파일 할때 blob을 만드는데 얘는 읽는거라 따로 만듬
...
void Shader::Pass::EndDraw()
{
	...
	ID3D11ShaderResourceView* arrViews[1] = { 0 };
	D3D::GetDC()->PSSetShaderResources(0, 1, arrViews);
	ID3D11Buffer* nullCB[1] = { 0 };
	D3D::GetDC()->PSSetConstantBuffers(0, 1, nullCB);
...
void Shader::Pass::Dispatch(UINT x, UINT y, UINT z)
{
	...
	ID3D11Buffer* nullCB[1] = { 0 };
	D3D::GetDC()->CSSetConstantBuffers(0, 1, nullCB);
...
void Shader::SetShaders(UINT technique, UINT pass)
{
	techniques[technique].Passes[pass].BeginDraw();
	techniques[technique].Passes[pass].EndDraw();
}
